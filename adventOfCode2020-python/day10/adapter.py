...
#  your seat produces the wrong number of jolts. 
...
#  output joltage (your puzzle input). Any given adapter can
...
# Treat the charging outlet near your seat as having 
# an effective joltage rating of 0.
# Since you have some time to kill, you might as well 
# test all of your adapters. Wouldn't want to get to your
#  resort and realize you can't even charge your device!

# If you use every adapter in your bag at once, what is 
# the distribution of joltage differences between the charging outlet, the adapters, and your device?
...
# it directly would need to have a joltage rating of 1, 2, or 3 jolts. Of these, only one you have is an adapter rated 1 jolt (difference of 1).
...
def findNumberOfAllowedArrangements(adapters: List[int]) -> int:
    ...
    for i in range(len(adapters)):
        if isNextSkippable(adapters, i):
            # print(f'skippable {adapters[i+1]}')
            numberOfSolutions *= 2
            possibleSkips += 1
    print(f'number of solutions {numberOfSolutions}, skips: {possibleSkips}')
